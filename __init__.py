...
class Player(BasePlayer):
    ...
    def play_round(self):
        ...
        if self.choice:  # Игрок рискнул
            self.success = random.random() < C.SUCCESS_RATE
            if self.success:
            ...
            self.total_payoff += self.current_amount  # Увеличиваем общий выигрыш
    ...
    def vars_for_template(player: Player):
        ...
        for p in player.in_all_rounds():
            all_rounds.append({
                ...
                'initial_amount': p.current_amount,  # Начальная сумма в раунде
                'choice': 'Рискнул' if p.choice else 'Забрал деньги',
                'final_amount': p.current_amount * 2 if (p.choice and p.success) else C.INITIAL_AMOUNT,  # Конечная сумма
                'total_payoff': p.total_payoff,  # Общий выигрыш к концу раунда
